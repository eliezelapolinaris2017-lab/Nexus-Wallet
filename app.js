const S = {
  ...
  services:[],
  contracts:[],
  cal: { view: new Date(), selectedISO: UI.dateISO(new Date()) }
};
...
function wireGlobalButtons(){
  ...
  UI.$("btnToday").addEventListener("click", ()=>{
    S.cal.view = new Date();
    S.cal.selectedISO = UI.dateISO(new Date());
    renderCalendar();
    loadAgendaNote();
  });
...
function openClientModal(client){
  ...
  if(editing){
    UI.$("mDel").addEventListener("click", async ()=>{
      ...
      // borra también registros vinculados
...
function openVisitModal(visit){
  ...
  // autocompletar precio según catálogo
  const refreshPrice = () => {
...
function wireAgenda(){
  UI.$("calPrev").addEventListener("click", ()=>{
    S.cal.view = new Date(S.cal.view.getFullYear(), S.cal.view.getMonth()-1, 1);
    renderCalendar();
  });
  ...
  UI.$("btnSaveAgenda").addEventListener("click", saveAgendaNote);
}

function renderCalendar(){
  const d = S.cal.view;
  UI.$("calTitle").textContent = UI.monthTitle(d).replace(/^./, c=>c.toUpperCase());
  ...
  const wrap = UI.$("calendar");
  wrap.innerHTML = cells.map(dt=>{
    if(!dt) return `<div class="day" style="opacity:.35"></div>`;
    const iso = UI.dateISO(dt);
    const sel = iso === S.cal.selectedISO;
    return `
      <div class="day ${sel?"sel":""}" data-iso="${iso}">
        <div class="dayNum">${dt.getDate()}</div>
        <div class="dayMark" id="mark-${iso}"></div>
      </div>
    `;
  }).join("");

  wrap.querySelectorAll("[data-iso]").forEach(cell=>{
    cell.addEventListener("click", ()=>{
      S.cal.selectedISO = cell.getAttribute("data-iso");
      renderCalendar();
      loadAgendaNote();
    });
  });

  // marcas: si hay nota ese día
  paintAgendaMarks();
}

async function paintAgendaMarks(){
  const all = await DB.getAll("agenda");
  const set = new Set(all.filter(x=>x.note && x.note.trim()).map(x=>x.id));
  set.forEach(iso=>{
    const el = document.getElementById(`mark-${iso}`);
    if(el) el.textContent = "• Nota";
  });
}

async function loadAgendaNote(){
  UI.$("agendaDate").textContent = S.cal.selectedISO;
  const rec = await DB.get("agenda", S.cal.selectedISO);
  UI.$("agendaNote").value = rec?.note || "";
}

async function saveAgendaNote(){
  const note = UI.$("agendaNote").value.trim();
  await DB.put("agenda", { id: S.cal.selectedISO, note, updatedAt: Date.now() });
  UI.showMsg(UI.$("agendaMsg"), "Guardado ✅", true);
  await paintAgendaMarks();
  setTimeout(()=>UI.showMsg(UI.$("agendaMsg"), ""), 1200);
}
...
function wireConfig(){
  ...
  UI.$("importFile").addEventListener("change", async (e)=>{
    ...
    try{
      ...
      await putAll("agenda", data.agenda);
